import { formatBusinessHoursList } from '../PickupBusinessHours'

describe('<PickupBusinessHours />', () => {
  describe('format business hours util', () => {
    ...
    it('should not skip days with different opening/closing hours', () => {
      const businessHours = [
        {
          DayOfWeek: 1,
      ...
      expect(result).toStrictEqual([
        expect.objectContaining({ closed: false, number: 1 }),
        expect.objectContaining({ closed: false, number: 2 }),
        expect.objectContaining({ closed: false, number: 3 }),
        expect.objectContaining({ closed: false, number: 4 }),
        expect.objectContaining({ closed: false, number: 5 }),
        expect.objectContaining({ closed: true, number: 6 }),
        expect.objectContaining({ closed: true, number: 0 }),
      ])
    })
  })
})
